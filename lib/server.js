  ...
  .get((req, res) => {
    ...
    request
      ...
      .then((response) => {
        ...
        res.status(200);
        const propsList = ['DateCreated', 'ACS_ClaimStatus', 'ACS_ClaimAmount', 'ACS_ApprovedAmount', 'ACS_IncidentCause',
          'ACS_PolicyHolderID', 'ACS_ApprovedAmount', 'ACS_ClaimArea'];
        const claimProps = [];
        for (let i = 0; i < result.Properties.length; i += 1) {
          const propName = result.Properties[i].SymbolicName;
